...
bool DISK_Initialize(DISK* disk, uint8_t driveNumber) {
    ...
    x86_64_outb(ATA_PRIMARY_DRIVE_HEAD, 0xA0);
    x86_64_iowait();

    // Perform a software reset
    x86_64_outb(ATA_PRIMARY_CONTROL, 0x04); // Set SRST (Software Reset)
    x86_64_iowait();
    x86_64_outb(ATA_PRIMARY_CONTROL, 0x00); // Clear SRST
    x86_64_iowait();

    // Wait for the drive to finish the reset.
    int timeout = 0x0FFFFFFF; // Increased timeout significantly
    while ((x86_64_inb(ATA_PRIMARY_STATUS) & ATA_STATUS_BUSY) && --timeout);
    if (timeout == 0) {
    ...
    // Zero out registers
    x86_64_outb(ATA_PRIMARY_SECTOR_COUNT, 0);
    x86_64_outb(ATA_PRIMARY_LBA_LOW, 0);
    x86_64_outb(ATA_PRIMARY_LBA_MID, 0);
    x86_64_outb(ATA_PRIMARY_LBA_HIGH, 0);

    // Send IDENTIFY DEVICE command
    x86_64_outb(ATA_PRIMARY_COMMAND, ATA_CMD_IDENTIFY_DEVICE);
    x86_64_iowait();
    ...
    timeout = 0x0FFFFFFF;
    while (!(x86_64_inb(ATA_PRIMARY_STATUS) & (ATA_STATUS_DATA_REQUEST | ATA_STATUS_ERROR)) && --timeout);

    if (timeout == 0) return false;

    if (x86_64_inb(ATA_PRIMARY_STATUS) & ATA_STATUS_ERROR) {
    ...
    x86_64_insw(ATA_PRIMARY_DATA, identify_data, 256);
...
bool DISK_ReadSectors(DISK* disk, uint32_t lba, uint8_t count, void* buffer) {
    ...
    x86_64_outb(ATA_PRIMARY_DRIVE_HEAD, 0xE0 | (disk->id << 4) | ((lba >> 24) & 0x0F));
    // Send the number of sectors to read
    x86_64_outb(ATA_PRIMARY_SECTOR_COUNT, count);

    // Send the LBA address (bits 0-23)
    x86_64_outb(ATA_PRIMARY_LBA_LOW, (uint8_t)lba);
    x86_64_outb(ATA_PRIMARY_LBA_MID, (uint8_t)(lba >> 8));
    x86_64_outb(ATA_PRIMARY_LBA_HIGH, (uint8_t)(lba >> 16));
    ...
    for (int i = 0; i < count; i++) {
        ...
        timeout = 0x0FFFFFFF;
        while (!((x86_64_inb(ATA_PRIMARY_STATUS) & ATA_STATUS_DATA_REQUEST) || (x86_64_inb(ATA_PRIMARY_STATUS) & ATA_STATUS_ERROR)) && --timeout);
        ...
        x86_64_insw(ATA_PRIMARY_DATA, target, 256);
        target += 256;
    }
    return true;
}
...
bool DISK_WriteSectors(DISK* disk, uint32_t lba, uint8_t count, const void* buffer) {
    ...
    x86_64_outb(ATA_PRIMARY_COMMAND, ATA_CMD_WRITE_SECTORS);
    ...
    for (int i = 0; i < count; i++) {
        ...
        x86_64_outsw(ATA_PRIMARY_DATA, source, 256);
        source += 256;
    }

    // Flush the cache to ensure data is written to the disk platter
    x86_64_outb(ATA_PRIMARY_COMMAND, ATA_CMD_CACHE_FLUSH);
    timeout = 0x0FFFFFFF;
    while ((x86_64_inb(ATA_PRIMARY_STATUS) & ATA_STATUS_BUSY) && --timeout);
